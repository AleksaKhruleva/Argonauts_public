...
struct ServiceMaterialView: View {
    ...
    func canAddMaterial() -> Bool {
        if matInfo.count <= 64 {
            if matCost.isEmpty && wrkCost.isEmpty {
                return true
            } else if matCost.isEmpty && !wrkCost.isEmpty {
    ...
    func getMaterial(sid: String) {
        let urlString = appURL + "?mission=get_material&sid=" + sid
    ...
    func addMaterial(sid: String, matInfo: String, wrkType: String, matCost: String, wrkCost: String) {
        let urlString = appURL + "?mission=add_material&sid=" + sid + "&mat_info=" + matInfo + "&wrk_type=" + wrkType + "&mat_cost=" + matCost + "&wrk_cost=" + wrkCost
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
    ...
    func deleteMaterial(maid: String) {
        let urlString = appURL + "?mission=delete_material&maid=" + maid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
