...
struct StatisticsDetailView: View {
    ...
    var body: some View {
                    ...
                    ScrollView(showsIndicators: false) {
                        ...
                        Group {
                            HStack {
                                ...
                                Spacer()
                                Text(String(describing: statistics[currItem].fuelCnt).replacingOccurrences(of: ".", with: ","))
                            }
                            Divider()
                            HStack {
                                Text("Средняя заправка")
                                Spacer()
                                Text(String(describing: statistics[currItem].fuelAvg).replacingOccurrences(of: ".", with: ","))
                            }
                            Divider()
                            HStack {
                                Text("Сум. заправка")
                                Spacer()
                                Text(String(describing: statistics[currItem].fuelSum).replacingOccurrences(of: ".", with: ","))
                            }
                            Divider()
                            HStack {
                                Text("Мин. заправка")
                                Spacer()
                                Text(String(describing: statistics[currItem].fuelMin).replacingOccurrences(of: ".", with: ","))
                            }
                            Divider()
                            HStack {
                                Text("Макс. заправка")
                                Spacer()
                                Text(String(describing: statistics[currItem].fuelMax).replacingOccurrences(of: ".", with: ","))
                            }
                        }
                        HStack {
                            Text("Пробег")
        ...
        .alert(isPresented: $showAlert) {
            if alertMessage == "Вы уверены, что хотите отправить статистику на все активные почты?" {
                return Alert(title: Text("Подтверждение"), message: Text(alertMessage), primaryButton: .cancel(), secondaryButton: .default(Text("Отправить")) {
                    sendStatisticsAsync()
                })
            } else {
    ...
    func getStatisticsMonth(tid: String) {
        let urlString = appURL + "?mission=get_statistics_month&tid=" + tid
    ...
    func getStatisticsYear(tid: String) {
        let urlString = appURL + "?mission=get_statistics_year&tid=" + tid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
    ...
    func sendStatisticsMonth(tid: String) {
        let urlString = appURL + "?mission=send_statistics_month&tid=" + tid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
    ...
    func sendStatisticsYear(tid: String) {
        let urlString = appURL + "?mission=send_statistics_year&tid=" + tid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
