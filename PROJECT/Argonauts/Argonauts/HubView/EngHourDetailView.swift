...
struct EngHourDetailView: View {
    ...
    func getEngHour(tid: String) {
        let urlString = appURL + "?mission=get_eng_hour&tid=" + tid
    ...
    func addEngHour(tid: String, date: Date, engHour: String) {
        ...
        let urlString = appURL + "?mission=add_eng_hour&tid=" + tid + "&date=" + dateString + "&eng_hour=" + engHour
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
    ...
    func deleteEngHour(ehid: String, tid: String) {
        let urlString = appURL + "?mission=delete_eng_hour&ehid=" + ehid + "&tid=" + tid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
