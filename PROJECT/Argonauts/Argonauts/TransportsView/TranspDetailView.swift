...
struct TranspDetailView: View {
    ...
    @State var keys: [String] = ["Ник", "Год выпуска", "Пробег", "Моточасы", "Дата диагностической\nкарты", "Дата ОСАГО", "Суммарный расход\nтоплива", "Дата отсчета\nтоплива"]
    ...
    var body: some View {
        ...
        .alert(isPresented: $showAlert) {
            if alertMessage == "Вы уверены, что хотите удалить данное транспортное средство?" {
                return Alert(title: Text("Внимание"), message: Text(alertMessage), primaryButton: .cancel() {
                    alertMessage = ""
    ...
    func discardFuel(tid: String) {
        let urlString = appURL + "?mission=discard_fuel&tid=" + tid
    ...
    func getTransp(tid: String) {
        let urlString = appURL + "?mission=get_transp&tid=" + tid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                    ...
                    if info[0]["server_error"] != nil {
                        ...
                        if info[0]["total_fuel"] is NSNull == false {
                            ...
                            values[6] = String(format: "%.2f", totalFuel).replacingOccurrences(of: ".", with: ",")
                        }
    ...
    func deleteTransp(tid: String) {
        let urlString = appURL + "?mission=delete_transp&tid=" + tid
        let encodedUrl = urlString.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)
        let url = URL(string: encodedUrl!)
        if let data = try? Data(contentsOf: url!) {
            do {
